...
namespace video {
  ...
  bool validate_encoder(encoder_t &encoder, bool expect_failure) {
    ...
    if (test_prores) {
      ...
      // feeds a 10-bit pix_fmt to prores_videotoolbox rather than the 8-bit
      // NV12 the H.264 / HEVC probes use.
      //
      // encoderCscMode = 3 (full range, BT.709): prores_videotoolbox rejects
      // BT.601 colorspace even at 10-bit, because ProRes was never intended
      // for SD content. BT.709 matches what the encoder actually expects.
      //
      // chromaSamplingType = 1 (4:4:4): prores_videotoolbox's supported
      // input pix_fmt list does not include 4:2:0 BiPlanar formats (P010)
      // for any profile — the 422 family (proxy / lt / standard / hq) wants
      // 4:2:2 or higher chroma, and the 4444 family wants 4:4:4 natively.
      // Feeding 4:4:4 P410 lets the encoder downsample to the chosen
      // profile internally.
      ...
      if (disp->is_codec_supported(encoder.prores.name, prores_autoselect)) {
        ...
        // PASSED and only sets DYNAMIC_RANGE itself, so promoting it eagerly
        // here keeps the encoder's capability advertisement consistent for
        // clients that opt into ProRes via prores_mode > 0.
  ...
  int probe_encoders() {
    ...
    // Derive require_prores from the immutable user-intent source
    // (config::video.prores_mode) rather than the mutable runtime
    // active_prores_mode, which adjust_encoder_constraints below can
    // zero out when an encoder lacks ProRes support. Tying this to
    // user intent keeps the "force ProRes" policy consistently
    // enforced across all encoder candidates in the search loop.
