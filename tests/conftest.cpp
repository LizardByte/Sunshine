...
class BaseTest : public ::testing::Test {
  protected:
    ...
    BaseTest() : sbuf{nullptr}, pipe_stdout{nullptr}, pipe_stderr{nullptr} {
      // intentionally empty
    }
    ...
    void TearDown() override {
      ...
      if (test_info->result()->Failed()) {
        std::cout << std::endl
                  << "Test failed: " << test_info->name() << std::endl << std::endl
    ...
    int exec(const char* cmd) {
      ...
      pipe_stdout = nullptr;
      if (returnCode != 0) {
        std::cout << "Error: " << stderr_buffer.str() << std::endl << "Return code: " << returnCode << std::endl;
      }
      return returnCode;
    }
};
