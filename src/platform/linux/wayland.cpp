...
#define CLASS_CALL(c, m) classCall<c, decltype(&c::m), &c::m>
  ...
  void dmabuf_t::linux_dmabuf(
    ...
    if (dmabuf_info.format != format) {
      char fourcc[5] {0};
      memcpy(&fourcc, &format, 4);
      BOOST_LOG(info) << "Screencopy DMA-BUF format: " << std::hex << format << " = " << fourcc;
    }
  ...
  void dmabuf_t::create_and_copy_dmabuf(zwlr_screencopy_frame_v1 *frame) {
    ...
    // Create GBM buffer
    if (!screencopy_session.supported_modifiers.empty()) {
      auto it = screencopy_session.supported_modifiers.find(dmabuf_info.format);
      if (it != screencopy_session.supported_modifiers.end() && !it->second.empty()) {
  ...
  struct buffer_feedback_format_table_entry {
    std::uint32_t format;
  ...
  void dmabuf_t::buffer_feedback_format_table(zwp_linux_dmabuf_feedback_v1 *feedback, int fd, int size) {
    ...
    void *map_data = mmap(nullptr, size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (map_data == MAP_FAILED) {
      return;
    }

    auto *entries = static_cast<const buffer_feedback_format_table_entry *>(map_data);
    std::size_t num_entries = size / sizeof(buffer_feedback_format_table_entry);

    char fourcc[5] {0};

    for (std::size_t i = 0; i < num_entries; ++i) {
      std::uint32_t format = entries[i].format;
      std::uint64_t modifier = entries[i].modifier;
      memcpy(&fourcc, &format, 4);
      BOOST_LOG(debug) << "Screencopy DMA-BUF format: " << std::hex << format << " = " << fourcc << ", modifier: " << modifier;
      screencopy_session.supported_modifiers[format].push_back(modifier);
    }
  ...
  void dmabuf_t::icc_create_and_copy_dmabuf(ext_image_copy_capture_frame_v1 *frame) {
    ...
    if (!icc_session.supported_modifiers.empty()) {
      auto it = icc_session.supported_modifiers.find(icc_session.format);
      if (it != icc_session.supported_modifiers.end() && !it->second.empty()) {
        current_bo = gbm_bo_create_with_modifiers(gbm_device, icc_session.width, icc_session.height, icc_session.format, it->second.data(), it->second.size());
      }
    }
    ...
    next_frame->sd.modifier = gbm_bo_get_modifier(current_bo);
  ...
  void dmabuf_t::icc_session_dmabuf_format(ext_image_copy_capture_session_v1 *session, std::uint32_t format, struct wl_array *modifiers) {
    ...
    if (icc_session.modifiers != nullptr) {
      ...
      for (size_t i = 0; i < icc_session.num_modifiers; ++i) {
        ...
        icc_session.supported_modifiers[format].push_back(icc_session.modifiers[i]);
      }
    }
