<template>
    ...
    <div class="card-body">
      ...
      <section class="virtual-gamepad-section">
        <div class="virtual-gamepad-section-heading">
          ...
          <button class="btn btn-outline-secondary driver-refresh-button" type="button"
            ...
            <tbody>
              <tr>
                ...
                <td class="driver-download-column">
                  <a class="btn btn-outline-primary driver-download-button" :href="virtualhidRelease.url"
              ...
              <tr id="vigembus" v-if="showVigembus">
                ...
                <td>
                  <div class="driver-release-state" :class="driverReleaseStateClass(vigembus, vigembusRelease)"
                    :title="driverReleaseStatusText(vigembus, vigembusRelease)" aria-live="polite">
                    <refresh-cw v-if="vigembusRelease.loading" :size="17" class="driver-release-spinning"></refresh-cw>
      ...
      <section class="virtualhid-license-section" aria-labelledby="virtualhid-license">
        ...
        <AlertBox v-if="licenseError" variant="danger" compact class="alert-inline">
          {{ licenseError }}
        </AlertBox>
        <AlertBox v-else-if="!virtualhidLicense.service_available" variant="warning" compact class="alert-inline">
          {{ virtualhidLicense.message || $t('troubleshooting.virtualhid_license_unavailable') }}
        </AlertBox>
        ...
        <p class="virtualhid-license-message" v-if="virtualhidLicense.message && virtualhidLicense.service_available">
        ...
        <div class="virtualhid-license-toolbar">
          ...
          <a class="btn btn-outline-primary" v-if="virtualhidLicense.manage_account_url"
            :href="virtualhidLicense.manage_account_url" target="_blank" rel="noopener noreferrer">
            <external-link :size="18"></external-link>
            {{ $t('troubleshooting.virtualhid_license_manage') }}
          </a>
          <a class="btn btn-success" v-if="virtualhidLicense.purchase_url && !virtualhidLicense.licensed"
            :href="virtualhidLicense.purchase_url" target="_blank" rel="noopener noreferrer">
            {{ $t('troubleshooting.virtualhid_license_buy') }}
          </a>
          <button class="btn btn-outline-danger" type="button" v-if="virtualhidLicense.licensed"
            :disabled="licenseBusy || !virtualhidLicense.service_available" @click="updateLicense('deactivate')">
        ...
        <div class="virtualhid-license-activation" v-if="!virtualhidLicense.licensed">
          ...
          <div class="input-group virtualhid-license-input">
            <input id="virtualhid-license-key" class="form-control" type="password" v-model="licenseKey"
              :placeholder="$t('troubleshooting.virtualhid_license_key_placeholder')"
              :disabled="licenseBusy || !virtualhidLicense.service_available" autocomplete="off" autocapitalize="off"
              spellcheck="false" @keyup.enter="updateLicense('activate')" />
  ...
  <div class="card my-4">
    <div class="card-body">
      ...
      <AlertBox v-if="closeAppStatus === true" variant="success" compact>
        {{ $t('troubleshooting.force_close_success') }}
      </AlertBox>
      <AlertBox v-if="closeAppStatus === false" variant="danger" compact>
  ...
  <div class="card my-4" v-if="platform === 'windows'">
    <div class="card-body">
      ...
      <AlertBox v-if="ddResetStatus === true" variant="success" compact>
        {{ $t('troubleshooting.dd_reset_success') }}
      </AlertBox>
      <AlertBox v-if="ddResetStatus === false" variant="danger" compact>
  ...
  <div class="card my-4">
    <div class="card-body">
      <h2 id="unpair">{{ $t('troubleshooting.unpair_title') }}</h2>
      ...
      <AlertBox v-if="unpairAllStatus === false" variant="danger" compact>
    ...
    <ul class="list-group list-group-flush" v-if="clients && clients.length > 0">
      <li v-for="client in clients" :key="client.uuid" class="list-group-item d-flex align-items-center">
        ...
        <div class="form-check form-switch ms-2 mb-0">
          <input class="form-check-input" type="checkbox" role="switch" :id="'toggle-' + client.uuid"
