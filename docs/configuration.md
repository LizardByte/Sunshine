...
<table>
    <tr>
        ...
        <td colspan="2">
            ...
            @note{ProRes is a constant-quality codec. When this mode is active the client-supplied bitrate
            target is effectively advisory — the encoder sizes each frame to preserve quality, and the actual
            stream rate is governed by content complexity and the selected `prores_profile`. Expect bursts
            well above the slider value (hundreds of Mbps at 4K for HQ/4444), and expect Moonlight to surface
            "reduce bitrate" / network-congestion warnings on links that are not effectively lossless. Use
            ProRes for local LAN testing and capture workflows, not for bandwidth-constrained streaming.}
        </td>
    </tr>
    <tr>
        <td>Default</td>
