...
__global__ void RGBA_to_NV12(
  ...
  // According to MSDN [1], MPEG-2 UV sampling scheme is preferred and the default interpretation
  // of chroma subpixels in Windows (and probably elsewhere). This means the UV subpixel is not 
  // in the center of the 2x2 block but aligned with the left column, and we only sample 2 UV pixels
  // per subpixel. Non-scientific testing (ie: eyballing) shows this 'looks better' so MSDN is 
  // probably correct.
  //
  // [1] https://learn.microsoft.com/en-gb/windows/win32/medfound/recommended-8-bit-yuv-formats-for-video-rendering?redirectedfrom=MSDN#yuv-sampling
  //
  float2 uv = (uv_lt + uv_lb) * 0.5f;
