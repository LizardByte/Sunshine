<template>
  <div class="container">
    ...
    <div class="card my-4">
      <div class="card-body">
        <div class="p-2">
          ...
          <div id="apply-alert" class="alert alert-success d-flex align-items-center mt-3"
            ...
            <button class="btn btn-success ms-auto apply" @click="clickedApplyBanner">{{ $t('_common.dismiss')
              }}</button>
          </div>
      ...
      <ul id="client-list" class="list-group list-group-flush list-group-item-light"
        v-if="clients && clients.length > 0">
        <div v-for="client in clients" class="list-group-item d-flex">
          <div class="p-2 flex-grow-1">{{ client.name !== "" ? client.name : $t('troubleshooting.unpair_single_unknown')
            }}</div>
